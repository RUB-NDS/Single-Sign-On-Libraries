...
import org.rub.nds.sso.exceptions.KeyException;
...
public final class SecurityUtils {
     ...
     * @return
     * @throws KeyException
     *             failing to create the SAML signature
     * @throws SAMLBuildException
     */
    public static Signature createSignature(final X509Certificate cert, final PrivateKey privKey) throws KeyException,
            SAMLBuildException {
        _log.info(SecurityUtils.class.toString()
                + ": Create SAML-Signature element for given certificate and privateKey object");
        ...
        try {
            ...
            keyInfo = (KeyInfo) Configuration.getBuilderFactory().getBuilder(KeyInfo.DEFAULT_ELEMENT_NAME)
     ...
     * @throws IOException
     */
    public static X509Certificate readCertificatefromPEM(final String filePath) throws CertificateException,
            IOException {
        InputStream is = null;
        X509Certificate crt = null;
        try {
            is = new FileInputStream(filePath); // filePath.getClass().getResourceAsStream("/"
                                                // + filePath);
     ...
     * @return
     * @throws KeyException
     *             extracting the keys from the specified Keystorage
     * @throws SAMLBuildException
     */
    public static Signature getSignatureFromFileJKS(final File keystore_path, final char[] key_keystore_pass,
            final char[] key_truststore_pass, String alias) throws KeyException, SAMLBuildException {
        ...
        try {
            ...
            pkEntry = (KeyStore.PrivateKeyEntry) keystore.getEntry(alias, new KeyStore.PasswordProtection(
                    key_keystore_pass));
            privatekey = pkEntry.getPrivateKey();

            // Retrieve certificate from KeyStorage:
            certificatex509 = getCertificateFromJKS(keystore, alias);
            // Create SAML signature with the help of the SAMLSignatureFactory
            // class:
    ...
    public static Signature copySignature(final Signature signSignature) throws ConfigurationException {
        ...
        try {
            ...
            Signature signature = (Signature) SAMLUtils.getSAMLBuilder().getBuilder(Signature.DEFAULT_ELEMENT_NAME)
                    .buildObject(Signature.DEFAULT_ELEMENT_NAME);
            signature.setSigningCredential(signSignature.getSigningCredential());
            signature.setSignatureAlgorithm(SignatureConstants.ALGO_ID_SIGNATURE_RSA_SHA1);
            signature.setCanonicalizationAlgorithm(SignatureConstants.ALGO_ID_C14N_EXCL_OMIT_COMMENTS);
            KeyInfoHelper.addCertificate(keyInfo,
                    ((BasicX509Credential) signature.getSigningCredential()).getEntityCertificate());

            signature.setKeyInfo(keyInfo);

            SecurityHelper.prepareSignatureParams(signature, signSignature.getSigningCredential(),
                    Configuration.getGlobalSecurityConfiguration(), keyInfoGeneratorProfile);
     ...
     * @throws KeyException
     */
    public static X509Certificate getCertificateFromJKS(final File keystore_path, final char[] key_keystore_pass,
            final char[] key_truststore_pass, String alias) throws KeyStoreException, KeyException {
        KeyStore keystore;
     ...
     * @throws KeyException
     */
    public static KeyStore getKeyStore(final File keystore_path, final char[] key_keystore_pass,
            final char[] key_truststore_pass, String alias) throws KeyException {
        KeyStore keystore;
        Enumeration enumer;
